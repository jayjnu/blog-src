...
- 메뉴판에 없는 주문은 받지 않으며, 다시 주문을 요청한다.
- 필수재료가 떨어지면 샌드위치는 즉시 매진 상태가 된다.
- 필수재료를 제외한 재료는 부족하지만 특별 주문을 받은경우 (예: 양파가 부족한 상황에서 필리치즈 주문시 양파는 제외하고 주라는 주문) 주문을 받는다.
- 빵은 총 두 가지: 호밀 바게뜨, 밀 바게뜨 (수량 제한 있음)
- 소스는 없다 (너무복잡해짐)
- 돈이 부족한 경우 재요청하고 두 번 부족하면 거래를 거절한다.
...
- 필리치즈스테이크
    ...
    - 가격: 8000
    - 레시피:
        1. 스테이크를 잘게 다지고 나서 140도에서 5초동안 볶는다.
        2. 양파를 슬라이스해서 자른 후 3초동안 볶는다.
